...
std::streamoff SearchString(fstream *file, std::streamoff pos, std::streamoff slen, const char *str, long strlen)
{
 std::streamoff  i;
 char buf[SBUFSIZE];
 ...
 std::streamoff  restbytes;	//length of string - 1
...
*******************************/
std::streamoff ExtractFile(fstream *srcf, std::streamoff srcofs, char *fname, std::streamoff nbytes)
{
 if(!do_extract) {
 ...
 strcpy_s(dstp, FNBUFSIZE, out_path);
 strcat_s(dstp, FNBUFSIZE, fname);
...
void mkfname(int val, int ndigits, char *fname, size_t sizeOfBuffer, const char *prefix, const char *ext)
{
 // the sprintf format string must be dynamically generated considering "ndigits"
 //  so we create it using another "sprintf()"
 const size_t LENGTH = 50;
 char formstring[LENGTH];
 sprintf_s(formstring, LENGTH, "%%s%%0%dd%%s", ndigits);  //"%%" results in '%' in the string
 sprintf_s(fname, sizeOfBuffer, formstring,   prefix, val,ext);
}
...
int createDir()
{
 ...
 if(strlen(out_path) == 0)
   strcat_s(out_path, FNBUFSIZE, "./");
 else {
   char c = out_path[ strlen(out_path) - 1];
   if( c != '/'  &&  c != '\\' )  //make sure string ends with slash
     strcat_s(out_path, FNBUFSIZE, "/");
 }
 strcat_s(out_path, FNBUFSIZE, "AUDIORIP_");
 
 do
 {
   mkfname(i, 3, xdir, FNBUFSIZE, out_path, xdir_suffix);
   error = _mkdir(xdir);
   if(((error != 0) && (errno != EEXIST)) || (i > 999)) {
       ...
       switch(errno) {
		 ...
		 {
			 char szError[512] = "";
			 strerror_s(szError, _countof(szError), errno);
			 cout << szError; break;
		 }
       }
       cout << endl;
 ...
 strcat_s(xdir, _countof(xdir),"/");
 strcpy_s(RgC(out_path), xdir);
 return 0;
}
