...
std::streamoff ripwav(fstream *file, std::streamoff filesize)
{
 ...
 while(1) {
   ...
   mkfname(cnt, 8, fname, FNBUFSIZE, "", ".wav");
...
std::streamoff ripogg(fstream *file, std::streamoff filesize)
{
 ...
 while(1)
 {
   ...
   //make file name
   mkfname(cnt, 8, fname, FNBUFSIZE, "", ".ogg");
   //extract audio data to new file
   if(ExtractFile(file, pos, fname, outsize) == -1)
     return -1;
   //update number
 ...
 */
 ...
 std::streamoff  framesize;
 std::streamoff  framecnt;      // frame counter for current mpeg stream
 ...
 long   channelmode;  // channel mode (should be constant within a stream)

 //for comparison with the first frame, must remain the same in one mpeg stream
 char first_type;
 char first_layer;
 long  first_samplerate;
 long  first_channelmode;   
 ...
 while(1) 
 {
   ...
   mkfname(cnt, 8, fname, FNBUFSIZE, "", ext);
   //extract audio data to new file
   if(ExtractFile(file, pos, fname, outsize) == -1)
     return -1;
